...
//	    printf("%s:%d, before select\n", __FILE__, __LINE__);
	    ...
	    if(selret < 0)
	    {
			if(errno == EINTR)
			{
				//printf("%s:%d, select failed,errno = %d 111\n", __FILE__, __LINE__,errno);
				ntrytimes++;
				continue;
			}
			else
...
//					printf("%s:%d, before  recv\n", __FILE__, __LINE__);
					...
					//printf("%s:%d, after recv,recv:%s  and retlen is %d, errno is %d\n", __FILE__, __LINE__, ptr,retlen, errno);
					if(retlen < 0)
					{
