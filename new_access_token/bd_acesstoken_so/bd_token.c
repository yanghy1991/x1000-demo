...
int bd_get_token(char* url,char *access_token)
{
    ...
    *pHost = '\0';
	...
	if(do_bdtoken_connect(socket_fd, (struct sockaddr *)&socaddr, &addrlen) == false)
    {
	...
	int sendlen = do_bdtoken_send(socket_fd, request);
	if(sendlen < 0)
	{
	...
	int recvlen = do_bdtoken_recv(socket_fd, response, TOKEN_RESP_BUFSIZE) ;
	if(recvlen < 0)
	{
...
*功能:请求http获取access_token
*参数:
*token_path:存储accesstoken目录路径
*返回:
*返回0说明成功;返回-1说明失败
*/
int get_bd_tokenaccess(char *token_path)
{
	//1.构建请求字符串
...
//	printf("token http : \n%s\n",request_content);
	...
	//3.存储accesstoken路径是否存在,不存在则创建
	if(is_dir_exist(token_path) == -1)
	{
		if(create_dir(token_path) != 0)
		{
	...
	//open file
	char filePath[256] = {0};
	strcpy(filePath,token_path);
	if(filePath[strlen(filePath)] != '/')
	{
		strcat(filePath,"/bdtoken.txt");
	}
	else
		strcat(filePath,"bdtoken.txt");
	...
	close(fd);
//	printf("get_bd_tokenaccess over ......\n");
	return 0;
}
