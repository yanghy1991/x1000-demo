...
*
*/
int rspmsg_parse_json(char *json_str,char *rspMsg)
{
	printf("json_str = %s\n",json_str);
	cJSON * root = NULL;
    cJSON * item = NULL;
...
*
*/
int get_msg_from_server(char* url,char *rspMsg)
{
    ...
    *pHost = '\0';
    ...
    socaddr.sin_addr.s_addr = inet_addr("192.168.64.1");
    ...
    int nCount;
	nCount = recv(socket_fd,response,ASR_RESP_BUFSIZE,0);
    if(-1 == nCount)
    {
    ...
    else
    {
		...
		else
		{
			...
			int re_num = rspmsg_parse_json(token_json,rspMsg);
			if(re_num == -1)
			{
