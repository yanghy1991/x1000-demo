...
int get_msg_from_server(char* url,char *rspMsg,char *rspType)
{
    ...
    *pHost = '\0';
    ...
    socaddr.sin_addr.s_addr = inet_addr("39.108.71.229");
 ...
 /*
	...
	int ret = setsockopt(socket_fd,SOL_SOCKET,SO_RCVTIMEO,(char *)&nNetTimeout,sizeof(int));
	if(ret == -1)
	{
 ...
 */
    //客户端接收服务器的返回信息
	static char response[ASR_RESP_BUFSIZE];
    memset(response,0,ASR_RESP_BUFSIZE);
    int nCount = recv(socket_fd, response, ASR_RESP_BUFSIZE, 0);
    if(-1 == nCount)
    {
    ...
    else
    {
		...
		else
		{
			...
			int re_num = rspmsg_parse_json(token_json,rspMsg,rspType);
			if(re_num == -1)
			{
				printf("%s : there is no rspMsg \n",__func__);
				return -1;//获取city失败
			}
		}
